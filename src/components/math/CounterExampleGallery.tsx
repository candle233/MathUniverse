...
export const counterExamples: CounterExampleItem[] = [
  {
    ...
    nameEn: 'Weierstrass Continuous Nowhere Differentiable Function',
    discipline: '实分析与分形几何',
    ...
    <div className="rounded-2xl border border-slate-800 bg-slate-950 overflow-hidden shadow-2xl p-6 space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-slate-800">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="font-bold text-slate-100 text-sm">
              数学反例与病态怪兽殿堂 (Mathematical Counterexamples & Monsters)
            </h3>
        ...
        <span className="text-xs font-mono text-rose-400 font-bold bg-rose-950/40 px-3 py-1.5 rounded-xl border border-rose-500/30">
          收录 {counterExamples.length} 个经典反例
        </span>
      </div>
      ...
      <div className="p-6 rounded-2xl bg-slate-900/60 border border-slate-800 space-y-5">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="text-xs px-2.5 py-0.5 rounded-full bg-rose-500/20 text-rose-300 border border-rose-500/40 font-semibold">
                {activeItem.disprovenOrMonster}
              </span>
              <span className="text-xs text-slate-400 font-mono">{activeItem.discipline}</span>
            </div>
            <h4 className="text-lg font-bold text-slate-100 mt-1">{activeItem.nameZh}</h4>
            <p className="text-xs text-slate-400 font-mono">{activeItem.nameEn}</p>
          </div>
        </div>
        ...
        <div className="p-3.5 rounded-xl bg-amber-950/20 border border-amber-500/30 text-xs text-amber-200 space-y-1">
          <div className="font-bold flex items-center gap-1.5 text-amber-300">
            <Lightbulb className="w-4 h-4" /> 被证伪或击碎的直觉猜想：
          </div>
          <p>{activeItem.targetTheoremOrConjecture}</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
          <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800/80 space-y-2">
            ...
            <p className="text-slate-300 leading-relaxed">{activeItem.whyCrucial}</p>
          </div>

          <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800/80 space-y-2">
            <h5 className="font-bold text-slate-200 flex items-center gap-1.5">
              <Sparkles className="w-3.5 h-3.5 text-emerald-400" />
              <span>对现代数学发展的深远影响</span>
            </h5>
            <p className="text-slate-300 leading-relaxed">{activeItem.significance}</p>
          </div>
        </div>
      </div>
    </div>
