...
const mockPullRequests: PullRequest[] = [
  {
    ...
    downvotes: 1,
    reviewers: [
      { name: 'David Hilbert', decision: 'APPROVED', comment: '证明严谨无误，复内积的共轭处理完全符合 Mathlib 规范。' },
    ],
  },
  {
    id: 'pr-102',
    ...
    downvotes: 0,
    reviewers: [
      { name: 'Henri Poincaré', decision: 'APPROVED', comment: '完美的拓扑注记。' },
    ],
  },
];
...
export default function PullRequestViewer() {
  ...
  const [selectedPr, setSelectedPr] = useState<PullRequest>(prs[0]);
      ...
      prev.map((pr) =>
        pr.id === prId
          ? {
              ...
              reviewers: [
                ...pr.reviewers,
                { name: 'You (Reviewer)', decision: 'APPROVED', comment: '人工同行评审通过，赞成合并。' },
              ],
            }
          : pr
      )
    ...
    <div className="rounded-2xl border border-slate-800 bg-slate-950 overflow-hidden shadow-2xl">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-4 bg-slate-900 border-b border-slate-800">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="font-bold text-slate-100 text-sm">Git 风格同行评审与合并请求 (Pull Requests)</h3>
            <p className="text-xs text-slate-400">去中心化的学术同行评审机制，支持 LaTeX 源码与逻辑依赖精确 Diff 对比</p>
          </div>
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  >
                    {pr.status === 'MERGED' ? '已合并 (Merged)' : '审核中 (Open)'}
                  </span>
                </div>

                <h4 className="font-semibold text-slate-200 text-xs mb-1 line-clamp-1">{pr.title}</h4>
                <p className="text-[11px] text-slate-400 mb-2">关联定理: {pr.nodeTitle}</p>

                <div className="flex items-center justify-between text-[11px] text-slate-500 pt-1.5 border-t border-slate-800/60">
                  <span>作者: {pr.author.name}</span>
                  <span>👍 {pr.upvotes} 票</span>
                </div>
              </div>
            );
          })}
        ...
        <div className="lg:col-span-7 p-5 bg-slate-900/30 flex flex-col justify-between space-y-4">
          <div className="space-y-4">
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs text-slate-400 font-mono">提交于 {selectedPr.createdAt}</span>
              </div>
              <h3 className="font-bold text-slate-100 text-base">{selectedPr.title}</h3>
              <p className="text-xs text-slate-300 mt-1 leading-relaxed">{selectedPr.description}</p>
            </div>
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-950 overflow-hidden text-xs">
              <div className="px-3.5 py-2 bg-slate-900 border-b border-slate-800 font-mono text-[11px] text-slate-400 flex items-center justify-between">
                <span>LaTeX 源码 Diff 对比 (Semantic Diff)</span>
            ...
            <div className="space-y-2">
              <div className="text-xs font-semibold text-slate-300 flex items-center gap-1">
                ...
                <span>同行审阅者意见 (Peer Review Records):</span>
              </div>
          ...
          <div className="pt-3 border-t border-slate-800 flex flex-wrap items-center justify-between gap-3">
            <div className="text-xs text-slate-400">
              当前投票数: <span className="text-emerald-400 font-bold font-mono">+{selectedPr.upvotes}</span>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <span>{userVoted === 'APPROVED' ? '已投票赞成' : '赞成并同意合并 (Approve PR)'}</span>
              </button>
            </div>
          </div>
        </div>
